...
switch(strtolower($_REQUEST['status'])){ //Status is overloaded
    ...
    case 'homolog':
        $status='open';
		$results_type=__('Em Homologa&ccedil;&atilde;o');
        break;
    case 'block':
        $status='open';
		$results_type=__('Bloqueados');
        break;
    case 'closed':
        $status='closed';
...
if (!strcasecmp(strtolower($_REQUEST['status']), 'homolog')){
	$qwhere.=' and status.id = 8 ';
}else if (!strcasecmp(strtolower($_REQUEST['status']), 'block')){
	$qwhere.=' and status.id = 9 ';
}else if (!strcasecmp(strtolower($_REQUEST['status']), 'open')){
	$qwhere.=' and status.id not in (8,9) ';
}else if (!strcasecmp(strtolower($_REQUEST['status']), 'assigned')){
	$qwhere.=' and status.id not in (8,9) ';
}
...
$qselect.=' ,IF(ticket.duedate IS NULL,IF(sla.id IS NULL, NULL, DATE_ADD(ticket.created, INTERVAL sla.grace_period HOUR)), ticket.duedate) as duedate '
         ...
         .' ,cdata.priority as priority_id, cdata.subject, cdata.ordem, pri.priority_desc, pri.priority_color'
         .' ,Format((select sum(l.hours) from gestor_labor l where l.ticket_id = ticket.ticket_id ),2) Hours';
    ...
    <thead>
        <tr>
            ...
            <th width="170">
				...
				-->
                <th width="60" <?php echo $ord_sort;?>>
                    <a <?php echo $ord_sort; ?> href="tickets.php?sort=ord&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                        title="<?php echo sprintf(__('Sort by %s %s'), __('Ordem'), __($negorder)); ?>"><?php echo __('Ordem');?></a></th>
			...
			<th width="60" <?php echo $hours_sort;?>>
				<a <?php echo $hours_sort; ?> href="tickets.php?sort=hours&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
					title="<?php echo sprintf(__('Sort by %s %s'), __('Horas'), __($negorder)); ?>"><?php echo __('Horas');?></a></th>
        </tr>
     </thead>
     <tbody>
        <?php
        ...
        if($res && ($num=count($results))):
            ...
            foreach ($results as $row) {
                ...
                if($showassigned) {
                    if($row['staff_id'])
                        $lc=sprintf('<span >%s</span>',Format::truncate($row['staff'],40));
                    elseif($row['team_id'])
                        $lc=sprintf('<span >%s</span>',Format::truncate($row['team'],40));
                    else
                        $lc=' ';
                }else{
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
				...
				-->
                <td class="nohover" align="center"><?php echo $row['ordem']; ?></td>

                <td nowrap>&nbsp;<?php echo $lc; ?></td>

                <td class="nohover" align="center"><?php echo $row['Hours']; ?></td>
            </tr>
            <?php
            } //end of while.

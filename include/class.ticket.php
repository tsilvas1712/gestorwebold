   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function getStaffStats($staff) {
        ...
        $sql =  'SELECT \'open\', count( ticket.ticket_id ) AS tickets '
                ...
                .'UNION SELECT \'homolog\', count( ticket.ticket_id ) AS tickets '
                .'FROM ' . TICKET_TABLE . ' ticket '
                .'INNER JOIN '.TICKET_STATUS_TABLE. ' status
                    ON (ticket.status_id=status.id
                            AND status.state=\'open\') '
                ...
                .'UNION SELECT \'block\', count( ticket.ticket_id ) AS tickets '
                .'FROM ' . TICKET_TABLE . ' ticket '
                .'INNER JOIN '.TICKET_STATUS_TABLE. ' status
                    ON (ticket.status_id=status.id
                            AND status.state=\'open\') '
                ...
                .'INNER JOIN '.TICKET_STATUS_TABLE. ' status
                    ON (ticket.status_id=status.id
                            AND status.state=\'open\') '
                .'WHERE status.id not in (8,9) and ticket.staff_id = ' . db_input($staff->getId()) . ' '
                . $where

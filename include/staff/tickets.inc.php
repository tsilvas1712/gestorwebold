...
case 'homolog':
    ...
    $results_type=__('Em Homologa&ccedil;&atilde;o');
        $tickets->filter(array('status_id'=>8));
...
case 'block':
    ...
    $results_type=__('Bloqueados');
        $tickets->filter(array('status_id'=>9));
    $queue_sort_options = array('priority,updated', 'updated',
        'priority,due', 'due', 'priority,created', 'answered', 'number',
        'hot');
    break;
...
case 'assigned':
    ...
    //$tickets->filter(Q::any(array('staff_id'=>$thisstaff->getId(),
    //                              Q::all(array('staff_id' => 0, 'team_id__gt' => 0)),
    //                             )
    //                       )
    //                 );

    $tickets->filter(Q::not(array('status_id'=>8)));
    $tickets->filter(Q::not(array('status_id'=>9)));
...
case 'search':
    ...
    elseif (isset($_GET['orgid']) && ($org = Organization::lookup($_GET['orgid']))) {
        ...
        $results_type='Pesquisa personalizada'
...
case 'number':
    ...
    //     )
    // ));
    $tickets->order_by($sort_dir ? 'ticket_id' : '-ticket_id');
    break;
...
'source', 'cdata__:priority__priority_color', 'cdata__:cdata__ordem', 'cdata__:priority__priority_desc', 'status_id', 'status__name', 'status__state', 'dept_id', 'dept__name', 'user__name', 'lastupdate', 'isanswered', 'staff__firstname', 'staff__lastname', 'team__name');
        ...
        <tr>
            ...
            if ($showassigned ) {
                ...
                if (!strcasecmp($status,'closed'))
                    ...
                    $queue_columns['assignee']['heading'] =  'Agente';
            } else {
            ...
            <tr id="<?php echo $T['ticket_id']; ?>">
                ...
                <!-- <td align="center" nowrap><?php echo Format::datetime($T[$date_col ?: 'lastupdate']) ?: $date_fallback; ?></td> -->
                 ...
                 <?php
                    ...
                    $current    = Ticket::Lookup($T['ticket_id']);
                    $estDueDate = $current->getEstDueDate();
                    $estDueDate = date("d/m/Y H:i", strtotime($estDueDate));

                    // print_r($estDueDate); exit;

                ?>

                <td align="center" nowrap><?php echo $estDueDate; ?></td>
...
<?php               if ($T['attachment_count'])
                ...
                <td class="nohover" align="center">
                    <?php echo $T['cdata__:cdata__ordem']; ?>
                </td>
                <?php
                //}
                ?>
